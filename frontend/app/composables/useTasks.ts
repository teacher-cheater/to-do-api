...
export function useTasks() {
  ...
  const meta = useState<Paginated<Task>['meta'] | null>('tasks:meta', () => null)
  ...
  async function fetchTasks(filters: TaskFilters = {}) {
    ...
    try {
      ...
      const res = await request<Paginated<Task>>(`/api/tasks${query ? `?${query}` : ''}`)
      tasks.value = res.data
      meta.value = res.meta
    } catch (e: any) {
      error.value = e?.data?.message || 'Не удалось загрузить список задач.'
    } finally {
      loading.value = false
    }
  }
